...
class ApateGraphFeatures:
    ...
    def __init__(self, training_data, convergence_threshold=0.0000001, alpha=0.85):
        ...
        :param training_data:
            ...
            IMPORTANT NOTE: it is currently expected that the IDs of both Card Holders and Merchants in
            the given training data nicely start at 0 for the very first transaction, and increment as
            we go from the top to the bottom of the dataset (obviously with repeated entries if a transaction
            involves a Card Holder and/or Merchant that was also seen earlier in the dataset).

            This is because Merchant and Card Holder IDs are used directly as indices for rows/columns in
            the constructed adjacency matrix and score vector.
        ...
        # TODO
        '''
        a more flexible implementation with fewer restrictions on Card Holder and Merchant IDs than
        described above for the training_data param would be useful. This would require an extra dict,
        mapping from Merchant IDs or Card Holder IDs to integers.
        '''
        ...
        self.num_c = training_data["CardID"].max() + 1
        self.num_m = training_data["MerchantID"].max() + 1
    ...
    def compute_trx_score(self, row, interval):
        ...
        if card < self.num_c and merchant < self.num_m:
            ...
            for nonzero_col in reversed(nonzero_cols):
                ...
                old_merchant_id = nonzero_rows_2[1] - self.num_t - self.num_c

                if old_merchant_id == merchant:
    ...
    def compute_A_entry(self, transaction_date, test_date, gamma, interval):
        ...
        t = 0
        if interval == "short":
            t = timedelta.seconds / 60          # use minutes
        elif interval == "medium":
            t = timedelta.seconds / (60 * 60)   # use hours
        elif interval == "long":
